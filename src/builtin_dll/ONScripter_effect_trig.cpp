...
void ONScripter::buildSinTable() {
    if (!sin_table) {
        ...
        for (int i = 0; i < TRIG_TABLE_SIZE; i++) {
            sin_table[i] =
                (int)(sin((float)i * M_PI * 2 / TRIG_TABLE_SIZE) * TRIG_FACTOR);
        }
    }
}
...
void ONScripter::buildCosTable() {
    if (!cos_table) {
        ...
        for (int i = 0; i < TRIG_TABLE_SIZE; i++) {
            cos_table[i] =
                (int)(cos((float)i * M_PI * 2 / TRIG_TABLE_SIZE) * TRIG_FACTOR);
        }
    }
}
...
void ONScripter::effectTrvswave(char *params, int duration) {
    ...
    if (effect_counter * 2 < duration) {
        ...
        wvlen =
            (Sint16)(1.0 /
                     (((1.0 / TRVSWAVE_WVLEN_END - 1.0 / TRVSWAVE_WVLEN_START) *
                       2 * effect_counter / duration) +
...
void ONScripter::effectWhirl(char *params, int duration) {
    ...
    int t =
        (effect_counter * (TRIG_TABLE_SIZE / 4) / duration) % TRIG_TABLE_SIZE;
    int rad_amp = (sin_table[t] + cos_table[t] - TRIG_FACTOR) *
                  (TRIG_TABLE_SIZE / 2) / TRIG_FACTOR;
    int rad_base =
        ((TRIG_FACTOR - cos_table[t]) * TRIG_TABLE_SIZE / TRIG_FACTOR) +
        rad_amp;
    ...
    for (int i = 0; i < screen_height; ++i) {
        for (int j = 0; j < screen_width; ++j, ++dst_buffer, ++whirl_buffer) {
            ...
            jj = (((x + x + 1) * cos_table[theta] -
                   (y + y + 1) * sin_table[theta]) /
                      TRIG_FACTOR -
                  1) /
                     2 +
                 CENTER_X;
            ii = (((x + x + 1) * sin_table[theta] +
                   (y + y + 1) * cos_table[theta]) /
                      TRIG_FACTOR -
                  1) /
                     2 +
                 CENTER_Y;
