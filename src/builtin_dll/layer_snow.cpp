 ...
 *
 *  layer_snow.cpp
 *    - Emulation of Takashi Toyama's "snow.dll" and "hana.dll" NScripter plugin
 * effect
 *
...
void FuruLayer::buildAmpTables() {
    ...
    for (int i = 1; i < N_FURU_ELEMENTS; ++i) amp[i] = amp[i - 1] * 0.8;

    for (int i = 0; i < N_FURU_ELEMENTS; ++i) {
        Element *cur = &elements[i];
        if (!cur->amp_table) cur->amp_table = new int[FURU_AMP_TABLE_SIZE];
...
char *FuruLayer::message(const char *message, int &ret_int) {
    ...
    if (!strncmp(message, "i|", 2)) {
        ...
        if (tumbling) {
            ...
            if (sscanf(message, "i|%d,%d,%d,%d,%d,%d", &tmp[0], &num_cells[0],
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    SDL_Surface *img =
                        loadImage(&buf[i][0], &has_alpha, ref_surface, reader);
                    ...
                    if (anim->pos.w > max_sp_w) max_sp_w = anim->pos.w;
                }
            }
        } else {
            // "Snow"
        ...
        for (int i = 0; i < N_FURU_ELEMENTS; i++)
            ret_int +=
                (elements[i].pend - elements[i].pstart + FURU_ELEMENT_BUFSIZE) %
        ...
        for (int i = 0; i < N_FURU_ELEMENTS; i++) elements[i].clear();
        initialized = false;
    }
    return ret_str;
}
...
void FuruLayer::update() {
    if (initialized && !paused) {
        ...
        for (int j = 0; j < N_FURU_ELEMENTS; ++j) {
            ...
            while (i != cur->pend) {
                cur->points[i].pt.x =
                    (cur->points[i].pt.x + wind + virt_w) % virt_w;
    ...
    if (initialized) {
        ...
        for (int j = 0; j < N_FURU_ELEMENTS; j++) {
            ...
            if (cur->sprite) {
                ...
                const int n = (cur->pend - cur->pstart + FURU_ELEMENT_BUFSIZE) %
                              FURU_ELEMENT_BUFSIZE;
                int p = cur->pstart;
                if (amplitude == 0) {
                    ...
                    for (int i = n; i > 0; i--) {
                        ...
                        cur->sprite->pos.x =
                            ((curpt->pt.x + virt_w) % virt_w) - max_sp_w;
                    ...
                    for (int i = n; i > 0; i--) {
                        OscPt *curpt = &cur->points[p];
                        ++p %= FURU_ELEMENT_BUFSIZE;
                        const int disp_angle =
                            (angle + curpt->base_angle + FURU_AMP_TABLE_SIZE) %
                            FURU_AMP_TABLE_SIZE;
                        cur->sprite->current_cell = curpt->pt.cell;
                        cur->sprite->pos.x =
                            ((curpt->pt.x + cur->amp_table[disp_angle] +
                              virt_w) %
                             virt_w) -
                            max_sp_w;
                        cur->sprite->pos.y = curpt->pt.y;
