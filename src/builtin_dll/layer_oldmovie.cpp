...
class Scratch {
   private:
    int offs;    // Tint of the line: 64 for light, -64 for dark, 0 for no
                 // scratch.
...
void Scratch::init(int level) {
    ...
    if ((rand() % 600) < level) {
        ...
        dx = rand() % 12 - 6;
        x2 = x1 -
             dx;  // The angle of the line is determined by the speed of motion.
    }
}
...
// Called each time the screen is refreshed.  Draws a simple line, without
// antialiasing.
void Scratch::draw(SDL_Surface* surface, SDL_Rect clip) {
    // Don't draw unless this scratch is visible and likely to pass through the
    // updated rectangle.
    ...
    while (y < clip.y + clip.h) {
        ...
        if (lx >= clip.x &&
            lx < clip.x + clip.w) {  // Only draw within the clipping rectangle.
            // Get pixel...
            Uint32* p = (Uint32*)((char*)surface->pixels + y * sp + lx * 4);
            const Uint32 c = *p;
            // ...add to or subtract from its colour...
            int c1 = (c & 0xff) + offs, c2 = ((c >> 8) & 0xff) + offs,
                c3 = ((c >> 16) & 0xff) + offs;
            if (c1 < 0)
                c1 = 0;
            else if (c1 > 255)
...
void OldMovieLayer::om_init() {
    ...
    for (int i = 0; i < MAX_NOISE; i++) {
        NoiseSurface[i] =
            AnimationInfo::allocSurface(width, height, ons.getTextureFormat());
...
static void BlurOnSurface(SDL_Surface* src, SDL_Surface* dst, SDL_Rect clip,
    ...
    SDL_LockSurface(src);
    SDL_LockSurface(dst);
    unsigned char* src1px =
        ((unsigned char*)src->pixels) + srcx * 4 + srcy * srcp;
    unsigned char* src2px =
        ((unsigned char*)src->pixels) + clip.x * 4 + clip.y * srcp;
    unsigned char* dstpx =
        ((unsigned char*)dst->pixels) + clip.x * 4 + clip.y * dstp;

    // If the vertical offset is positive, we are reading one copy from (x, -1),
    // so we need to skip the first scanline to avoid reading outside the source
    // surface.
    ...
    // If the horizontal offset is -1, the rightmost column has not been written
    // to. Rectify that by copying it directly from the source image.
    ...
    // If we skipped the first scanlines, rectify that by copying directly from
    // the source image.
    if (skipfirstrows) {
...
// Draws the background image with the old-movie effect applied, using the
// settings adopted at the last call to updateOldMovie().
void OldMovieLayer::refresh(SDL_Surface* surface, SDL_Rect& clip) {
    ...
    // If an offset is present, we average the given surface with an offset
    // version
    ...
    unsigned char* g = (unsigned char*)GlowSurface->pixels +
                       (gv * glow_level / 4) * GlowSurface->pitch;
    const int sp = surface->pitch;
    if ((clip.x == 0) && (clip.y == 0) && (clip.w == width) &&
        (clip.h == height)) {
        // If no clipping rectangle is defined, we can apply the noise in one
        // go.
        ...
        if (noise_level > 0) {
            ...
            unsigned char* s =
                ((unsigned char*)surface->pixels) + clip.x * 4 + clip.y * sp;
            unsigned char* n = ((unsigned char*)NoiseSurface[ns]->pixels) +
                               clip.x * 4 + clip.y * np;
