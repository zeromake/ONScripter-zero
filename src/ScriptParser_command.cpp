...
int ScriptParser::useescspcCommand() {
    ...
    if (!force_button_shortcut_flag) useescspc_flag = true;
...
int ScriptParser::setlayerCommand() {
    ...
    if ((bslash && !strncmp(bslash + 1, "oldmovie.dll", 12)) ||
        ...
        handler =
            new FuruLayer(screen_width, screen_height, false, script_h.cBR);
    } else if ((bslash && !strncmp(bslash + 1, "hana.dll", 8)) ||
               !strncmp(dll, "hana.dll", 8)) {
        handler =
            new FuruLayer(screen_width, screen_height, true, script_h.cBR);
    } else {
        utils::printError("setlayer: layer effect '%s' is not implemented.",
                          dll);
        return RET_CONTINUE;
    }
...
int ScriptParser::rubyonCommand() {
    ...
    if (buf[0] == '\n' || buf[0] == ':' || buf[0] == ';') {
        ...
        ruby_struct.font_size_xy[0] =
            calcFontSize(script_h.readInt(), ons_font::RUBY_FONT);
        ruby_struct.font_size_xy[1] =
            calcFontSize(script_h.readInt(), ons_font::RUBY_FONT);
...
int ScriptParser::movCommand() {
    ...
    if (script_h.isName("mov10")) {
        ...
        count = -1;  // infinite
    } else if (script_h.getStringBuffer()[3] >= '3' &&
               script_h.getStringBuffer()[3] <= '9') {
...
int ScriptParser::menusetwindowCommand() {
    ...
    menu_font.font_size_xy[0] =
        calcFontSize(calcUserRatio(script_h.readInt()), menu_font.types);
    menu_font.font_size_xy[1] =
        calcFontSize(calcUserRatio(script_h.readInt()), menu_font.types);
    menu_font.pitch_xy[0] =
        calcUserRatio(script_h.readInt()) + menu_font.font_size_xy[0];
    menu_font.pitch_xy[1] =
        calcUserRatio(script_h.readInt()) + menu_font.font_size_xy[1];
    ...
    if (strlen(buf)) {  // Comma may or may not be appeared in this case.
        ...
        menu_font.window_color[0] = menu_font.window_color[1] =
            menu_font.window_color[2] = 0x99;
    }
...
int ScriptParser::lookbackspCommand() {
    ...
    for (int i = 0; i < 2; i++) lookback_sp[i] = script_h.readInt();
...
int ScriptParser::itoaCommand() {
    ...
    if (script_h.isName("itoa2")) itoa2_flag = true;
...
int ScriptParser::ifCommand() {
    if ((script_h.isName("elif") || script_h.isName("elseif")) &&
    ...
    while (1) {
        if (script_h.compareString("fchk")) {
            ...
            f = (script_h.findAndAddLog(
                     script_h.log_info[ScriptHandler::FILE_LOG], buf, false) !=
                 NULL);
            // utils::printInfo("fchk %s(%d) ", tmp_string_buffer,
            // (findAndAddFileLog( tmp_string_buffer, fasle )) );
        ...
        if (op_buf[0] == '|') {
            if (condition_status == 1)
                errorAndExit(
                    "if: using & and | at the same time is not supported.");
            while (*op_buf == '|') op_buf++;
...
int ScriptParser::getparamCommand() {
    ...
    do {
        ...
        if (script_h.pushed_variable.type & ScriptHandler::VAR_PTR) {
            ...
            const char *buf = script_h.readStr();
            setStr(
                &script_h.getVariableData(script_h.pushed_variable.var_no).str,
                buf);
        }
    ...
    if (getparam2_flag) {
        while (end_status2 & ScriptHandler::END_COMMA) {
            ...
            } else if (script_h.current_variable.type &
                           ScriptHandler::VAR_INT ||
                       script_h.current_variable.type &
                           ScriptHandler::VAR_ARRAY) {
                script_h.setInt(&script_h.current_variable, 0);
            } else if (script_h.current_variable.type &
                       ScriptHandler::VAR_STR) {
                setStr(
                    &script_h.getVariableData(script_h.current_variable.var_no)
                         .str,
                    NULL);
            }
        }
