...
AVIWrapper::~AVIWrapper() {
    if (v_stream) v_stream->StopStreaming();
    if (a_stream) a_stream->StopStreaming();
...
int AVIWrapper::initAV(SDL_Surface *surface, bool audio_open_flag) {
    ...
    if (cap & avm::IVideoDecoder::CAP_YV12) {
        ...
        screen_overlay =
            SDL_CreateYUVOverlay(width, height, SDL_YV12_OVERLAY, surface);
    } else if (cap & avm::IVideoDecoder::CAP_YUY2) {
        v_stream->GetVideoDecoder()->SetDestFmt(0, fccYUY2);
        screen_overlay =
            SDL_CreateYUVOverlay(width, height, SDL_YUY2_OVERLAY, surface);
    } else {
        screen_overlay =
            SDL_CreateYUVOverlay(width, height, SDL_YV12_OVERLAY, surface);
    }
...
int AVIWrapper::playVideo(void *userdata) {
    ...
    while (status == AVI_PLAYING && !v_stream->Eof()) {
        ...
        // getAudioTime(), minimum_time, nearest_cache, minimum_time,
        // current_time );
        if (async < -0.01) {
            if (remaining_cache == 0) {
                // printf("sync0 %f %f %f %f\n", async,
                // (a_stream)?a_stream->GetTime():0.0, v_stream->GetTime(),
                // minimum_time );
        ...
        if (nearest_cache >= 0 && minimum_time == cache[nearest_cache].time) {
            ...
            // cache[nearest_cache].time ); if ( async <= 0.033 ) // drop frame
            // if necessary
...
int AVIWrapper::play(bool click_flag) {
    ...
    time_start = 0;
    status = AVI_PLAYING;
    if (v_stream) thread_id = SDL_CreateThread(::playVideo, this);
    if (a_stream) Mix_HookMusic(::audioCallback, this);
    ...
    while (!(done_flag & click_flag) && status == AVI_PLAYING) {
        ...
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                case SDL_KEYUP:
                    if (((SDL_KeyboardEvent *)&event)->keysym.sym ==
                            SDLK_RETURN ||
    ...
    status = AVI_STOP;
    if (v_stream) SDL_WaitThread(thread_id, NULL);
    if (a_stream) Mix_HookMusic(NULL, NULL);
...
int AVIWrapper::drawFrame(avm::CImage *image) {
    ...
    if (comp == 0) {  // BGR
        image->ToYUV();
        for (i = 0; i < width * height; i++) *dst_y++ = buf[i * 3];
