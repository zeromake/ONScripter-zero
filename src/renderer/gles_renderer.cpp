...
static void casSetup(GLfloat con[4 * 2],
    ...
    con[2] =
        0.5f * inputSizeInPixels[0] * (1.0f / outputSizeInPixels[0]) - 0.5f;
    con[3] =
        0.5f * inputSizeInPixels[1] * (1.0f / outputSizeInPixels[1]) - 0.5f;
    // Sharpness value.
...
GLuint GlesRenderer::createShader(const GLenum shader_type,
