...
#define VERSION_STR2                                                         \
    "Copyright (C) 2001-2019 Studio O.G.A. All Rights Reserved.\n          " \
    "(C) 2014-2019 jh10001"
...
ScriptParser::ScriptParser() {
    ...
    int i;
    for (i = 0; i < CLICKVOICE_NUM; i++) clickvoice_file_name[i] = NULL;
    for (i = 0; i < SELECTVOICE_NUM; i++) selectvoice_file_name[i] = NULL;
...
void ScriptParser::reset(bool isDestroy) {
    ...
    /* ---------------------------------------- */
    /* Sound related variables */
    for (i = 0; i < CLICKVOICE_NUM; i++) setStr(&clickvoice_file_name[i], NULL);
...
void ScriptParser::writeChar(char c, bool output_flag) {
    if (output_flag) file_io_buf[file_io_buf_ptr] = (unsigned char)c;
    file_io_buf_ptr++;
}
...
int ScriptParser::readInt() {
    ...
    int i = (unsigned int)file_io_buf[file_io_buf_ptr + 3] << 24 |
...
void ScriptParser::writeStr(char *s, bool output_flag) {
    if (s && s[0]) {
        if (output_flag) memcpy(file_io_buf + file_io_buf_ptr, s, strlen(s));
...
void ScriptParser::writeArrayVariable(bool output_flag) {
    ...
    while (av) {
        int i, dim = 1;
        for (i = 0; i < av->num_dim; i++) dim *= av->dim[i];

        for (i = 0; i < dim; i++) {
            unsigned long ch = av->data[i];
            if (output_flag) {
                file_io_buf[file_io_buf_ptr + 3] =
                    (unsigned char)((ch >> 24) & 0xff);
                file_io_buf[file_io_buf_ptr + 2] =
                    (unsigned char)((ch >> 16) & 0xff);
                file_io_buf[file_io_buf_ptr + 1] =
...
void ScriptParser::readToken() {
    ...
    if (script_h.isText() && linepage_mode > 0) {
        char ch = '@';  // click wait
        if (linepage_mode == 1 ||
            sentence_font.getRemainingLine() <= clickstr_line)
            ch = '\\';  // newline
...
void ScriptParser::setKinsoku(const char *start_chrs,
    ...
    if (add) {
        ...
        if (start_kinsoku != NULL) delete[] start_kinsoku;
        tmp = new Kinsoku[1];
        num_start_kinsoku = 0;
    }
    ...
    if (add) {
        ...
        if (end_kinsoku != NULL) delete[] end_kinsoku;
        tmp = new Kinsoku[1];
        num_end_kinsoku = 0;
    }
