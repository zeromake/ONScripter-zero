...
SDL_Surface *ONScripter::loadImage(const char *filename,
                                   bool *has_alpha,
                                   int *location,
                                   unsigned char *alpha,
...
SDL_Surface *ONScripter::createSurfaceFromFile(const char *_filename,
                                               bool *has_alpha,
                                               int *location,
                                               const SDL_Point *load_size) {
...
#ifdef USE_IMAGE_CACHE
        if (!load_size) {
            imageBufferCache->Put(filename, buffer);
        }
#endif
    }
    SDL_RWops *src = SDL_RWFromMem(buffer->data(), buffer->size());
...
#define BLEND_PIXEL_MASK()                                                 \
    {                                                                      \
        ...
        Uint32 mask_rb =                                                   \
            (((((*src2_buffer & 0xff00ff) - temp) * mask2) >> 8) + temp) & \
            0xff00ff;                                                      \
        temp = *src1_buffer & 0x00ff00;                                    \
        Uint32 mask_g =                                                    \
            (((((*src2_buffer & 0x00ff00) - temp) * mask2) >> 8) + temp) & \
            0x00ff00;                                                      \
...
inline static void alphaBlendConst32(Uint32 *src1_buffer,
    ...
    for (int i = 0; i < remain;
         ++i, ++src1_buffer, ++src2_buffer, ++dst_buffer) {
        Uint32 temp = *src1_buffer & 0xff00ff;
        Uint32 mask_rb =
            (((((*src2_buffer & 0xff00ff) - temp) * mask2) >> 8) + temp) &
            0xff00ff;
...
void ONScripter::alphaBlend(SDL_Surface *mask_surface,
    ...
    if ((trans_mode == ALPHA_BLEND_FADE_MASK ||
        ...
        mask_surface) {
        struct Blender {
            ONSBuf *const stsrc1_buffer, *const stsrc2_buffer,
                                             *const stdst_buffer;
            int screen_width;
    ...
    {                                                                       \
        ...
        if (mask2 == 255) {                                                 \
            *dst_buffer = src_color3;                                       \
            ...
            Uint32 mask_rb =                                                \
                (((*dst_buffer & 0xff00ff) * mask1 + src_color1 * mask2) >> \
                 8) &                                                       \
                0xff00ff;                                                   \
            Uint32 mask_g =                                                 \
                (((*dst_buffer & 0x00ff00) * mask1 + src_color2 * mask2) >> \
                 8) &                                                       \
                0x00ff00;                                                   \
...
void ONScripter::alphaBlendText(SDL_Surface *dst_surface,
    ...
    if (fmt->BitsPerPixel == 16) {
        ...
        Uint16 *dst_buffer = (Uint16 *)dst_surface->pixels +
                             dst_surface->w * dst_rect.y + dst_rect.x;

        if (!rotate_flag) {
            unsigned char *src_buffer = (unsigned char *)src_surface->pixels +
                                        src_surface->pitch * y2 + x2;
            ...
            for (int i = 0; i < dst_rect.h; i++) {
                unsigned char *src_buffer =
                    (unsigned char *)src_surface->pixels +
                    src_surface->pitch * (src_surface->h - x2 - 1) + y2 + i;
                for (int j = dst_rect.w; j != 0; j--) {
                    BLEND_PIXEL_TEXT_BPP16();
...
void ONScripter::makeNegaSurface(SDL_Surface *surface, SDL_Rect &clip) {
    ...
    ONSBuf mask = surface->format->Rmask | surface->format->Gmask |
                  surface->format->Bmask;
    for (int i = clip.y; i < clip.y + clip.h; i++) {
        for (int j = clip.x; j < clip.x + clip.w; j++) *buf++ ^= mask;
    ...
    for (int i = clip.y; i < clip.y + clip.h; i++) {
        for (int j = clip.x; j < clip.x + clip.w; j++) {
            c = ((((*buf & fmt->Rmask) >> fmt->Rshift) << fmt->Rloss) * 77 +
                ...
                8;
            *buf++ = ((monocro_color_lut[c][0] >> fmt->Rloss)
                          << surface->format->Rshift |
                      (monocro_color_lut[c][1] >> fmt->Gloss)
                          << surface->format->Gshift |
                      (monocro_color_lut[c][2] >> fmt->Bloss)
...
void ONScripter::refreshSurface(SDL_Surface *surface,
    ...
    if (!(refresh_mode & REFRESH_SAYA_MODE)) {
        for (i = 0; i < MAX_PARAM_NUM; i++) {
            if (bar_info[i]) drawTaggedSurface(surface, bar_info[i], clip);
        }
        for (i = 0; i < MAX_PARAM_NUM; i++) {
            if (prnum_info[i]) drawTaggedSurface(surface, prnum_info[i], clip);
        }
    }
    ...
    if (show_dialog_flag) drawTaggedSurface(surface, &dialog_info, clip);
