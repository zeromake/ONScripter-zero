...
int ONScripter::playSound(const char *filename,
    ...
    if (format & SOUND_MUSIC) {
        ...
        if (Mix_FadeInMusic(
                music_info,
                (music_play_loop_flag && music_loopback_offset == 0.0) ? -1 : 0,
                fadetime) == 0) {
            music_buffer = buffer;
    ...
    /* check WMA */
    if (buffer[0] == 0x30 && buffer[1] == 0x26 && buffer[2] == 0xb2 &&
        buffer[3] == 0x75) {
        delete[] buffer;
        return SOUND_OTHER;
    }
...
void ONScripter::playCDAudio() {
    if (cdaudio_flag) {
        ...
        if (cdrom_info) {
            ...
            timer_cdaudio_id =
                SDL_AddTimer(length * 1000, cdaudioCallback, NULL);
        }
#endif
    } else {
...
int ONScripter::playMPEG(const char *filename,
    ...
    SMPEG_Info info;
    layer_smpeg_sample =
        SMPEG_new_rwops(SDL_RWFromMem(layer_smpeg_buffer, length), &info, 0);
    ...
    Uint8 *pixel_buf = new Uint8[info.width * info.height +
                                 (info.width / 2) * (info.height / 2) * 2];
    pixels[0] = pixel_buf;
    pixels[1] = pixel_buf + info.width * info.height;
    pixels[2] = pixel_buf + info.width * info.height +
    ...
    while (!(done_flag & click_flag) &&
           SMPEG_status(layer_smpeg_sample) == SMPEG_PLAYING) {
        SDL_Event event;
...
void ONScripter::stopBGM(bool continue_flag) {
    ...
    if (cdaudio_flag && cdrom_info) {
        ...
        if (SDL_CDStatus(cdrom_info) >= CD_PLAYING) SDL_CDStop(cdrom_info);
    }
#endif
