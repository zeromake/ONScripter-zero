...
int ONScripter::loadSaveFile2(int file_version) {
    ...
    for (i = 0; i < 3; i++) sentence_font.window_color[2 - i] = readChar();
    if (readChar() == 0x00)
    ...
    for (i = 0; i < MAX_SPRITE_NUM; i++) {
        ...
        if (file_version >= 203) ai->trans = readInt();
    }
    ...
    if (readInt() == 1)
        ...
        monocro_flag = false;
    for (i = 0; i < 3; i++) monocro_color[2 - i] = readInt();
    for (i = 0; i < 256; i++) {
    ...
    if (file_version >= 202) readArrayVariable();
...
void ONScripter::saveSaveFile2(bool output_flag) {
    ...
    writeInt(sentence_font_info.orig_pos.w + sentence_font_info.orig_pos.x - 1,
             output_flag);
    writeInt(sentence_font_info.orig_pos.h + sentence_font_info.orig_pos.y - 1,
             output_flag);
    ...
    for (i = 0; i < 3; i++) writeStr(tachi_info[i].image_name, output_flag);

    for (i = 0; i < 3; i++) writeInt(tachi_info[i].orig_pos.x, output_flag);

    for (i = 0; i < 3; i++) writeInt(tachi_info[i].orig_pos.y, output_flag);
    ...
    for (i = 0; i < 3; i++) writeInt(monocro_color[2 - i], output_flag);
    ...
    writeInt((midi_play_loop_flag) ? 1 : 0,
             output_flag);  // play, playonce MIDI
    ...
    writeInt(current_label_info.start_line + current_line + current_line_offset,
             output_flag);
