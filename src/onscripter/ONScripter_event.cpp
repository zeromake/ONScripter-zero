...
SDL_KeyboardEvent transJoystickAxis(SDL_JoyAxisEvent &jaxis) {
    ...
    if (jaxis.axis < 2) {
        axis = ((3200 > jaxis.value) && (jaxis.value > -3200)
                    ? -1
    ...
    if (event.type == ONS_MUSIC_EVENT) {
        ...
        if (tmp < cur_fade_duration) {
            ...
            if (ext && (strcmp(ext + 1, "OGG") && strcmp(ext + 1, "ogg"))) {
                // set break event to return to script processing when playing
                // music other than ogg
                SDL_Event event;
...
void ONScripter::flushEvent() {
    SDL_Event event;
    while (SDL_PollEvent(&event)) flushEventSub(event);
}
...
bool ONScripter::mouseMoveEvent(SDL_MouseMotionEvent *event) {
    ...
    if (event_mode & WAIT_BUTTON_MODE) {
        ...
        if (getmouseover_flag && current_over_button >= getmouseover_lower &&
...
bool ONScripter::mousePressEvent(SDL_MouseButtonEvent *event) {
    ...
    if (event_mode & (WAIT_INPUT_MODE | WAIT_BUTTON_MODE)) {
        if (!(event_mode & (WAIT_TEXT_MODE))) skip_mode |= SKIP_TO_EOL;
        playClickVoice();
        stopAnimation(clickstr_state);
        ...
        case SDLK_KP_ENTER:
            switch (variable_edit_mode) {
                case EDIT_VARIABLE_INDEX_MODE:
                    ...
                    variable_edit_num =
                        script_h.getVariableData(variable_edit_index).num;
                ...
                case EDIT_SE_VOLUME_MODE:
                    ...
                    if (wave_sample[MIX_LOOPBGM_CHANNEL0])
                        Mix_Volume(MIX_LOOPBGM_CHANNEL0,
                                   se_volume * MIX_MAX_VOLUME / 100);
                    if (wave_sample[MIX_LOOPBGM_CHANNEL1])
                        Mix_Volume(MIX_LOOPBGM_CHANNEL1,
                                   se_volume * MIX_MAX_VOLUME / 100);
                    break;
...
void ONScripter::shiftCursorOnButton(int diff) {
    ...
    for (num = 0; button; num++) button = button->next;
...
bool ONScripter::keyPressEvent(SDL_KeyboardEvent *event) {
    ...
    if (event->type == SDL_KEYUP) skip_mode &= ~SKIP_NORMAL;
    ...
    if ((trap_mode & TRAP_LEFT_CLICK) && (event->keysym.sym == SDLK_RETURN ||
    ...
    if ((event_mode & (WAIT_INPUT_MODE | WAIT_BUTTON_MODE)) &&
        ...
        } else if (!spclclk_flag && useescspc_flag &&
                   event->keysym.sym == SDLK_SPACE) {
...
void ONScripter::runEventLoop() {
    ...
    while (SDL_WaitEvent(&event)) {
        ...
        if (layer_smpeg_sample) SMPEG_status(layer_smpeg_sample);
#endif
        bool ret = false;
        ...
        switch (event.type) {
            ...
            case SDL_FINGERMOTION: {
                ...
                // render_view_rect.x) * screen_scale_ratio1; tmp_event.motion.y
                // = (device_height * event.tfinger.y - render_view_rect.y) *
                // screen_scale_ratio2; if (mouseMoveEvent( &tmp_event.motion ))
                // return; if (btndown_flag){
            ...
            case SDL_FINGERDOWN: {
                ...
                if (num_fingers == 2) {
                    ...
                    tmp_event.button.x =
                        (device_width * event.tfinger.x - render_view_rect.x) *
                        screen_scale_ratio1;
                    tmp_event.button.y =
                        (device_height * event.tfinger.y - render_view_rect.y) *
                        screen_scale_ratio2;
            ...
            case SDL_JOYBUTTONDOWN:
                ...
                event.key.keysym.sym =
                    transJoystickButton(event.jbutton.button);
                if (event.key.keysym.sym == SDLK_UNKNOWN) break;
            ...
            case SDL_JOYAXISMOTION: {
                ...
                if (ke.keysym.sym != SDLK_UNKNOWN) {
                    if (ke.type == SDL_KEYDOWN) {
                        keyDownEvent(&ke);
                        if (btndown_flag) keyPressEvent(&ke);
