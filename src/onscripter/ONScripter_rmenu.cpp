        ...
        switch (system_menu_mode) {
            ...
            default:
                if (system_menu_mode >= 1000) {
                    gosubReal(rmenu_calls.at(system_menu_mode),
                              script_h.getNext());
                    leaveSystemCall();
                    return 1;
                } else {
...
void ONScripter::executeSystemMenu() {
    ...
    while (link) {
        ButtonLink *button =
            getSelectableSentence(link->label, &menu_font, false);
...
bool ONScripter::executeSystemLoad() {
    ...
    for (unsigned int i = 1; i <= num_save_file; i++) {
        ...
        ButtonLink *button =
            getSelectableSentence(buffer, &menu_font, false, nofile_flag);
        root_button_link.insert(button);
        button->no = i;
        flush(refreshMode());
    }
...
void ONScripter::setupLookbackButton() {
    ...
    if ((current_page->previous->text_count > 0) &&
        ...
        if (lookback_sp[0] >= 0) {
            ...
            button->anim[0]->pos.x = button->anim[1]->pos.x =
                button->image_rect.x;
            button->anim[0]->pos.y = button->anim[1]->pos.y =
                button->image_rect.y;
        }
...
void ONScripter::executeSystemLookback() {
    ...
    if (current_page->text_count == 0) {
        if (lookback_sp[0] >= 0) sprite_info[lookback_sp[0]].visible = false;
        if (lookback_sp[1] >= 0) sprite_info[lookback_sp[1]].visible = false;
        leaveSystemCall();
        return;
    }
    ...
    while (1) {
        ...
        if (current_button_state.button == 0 ||
            (current_page == start_page && current_button_state.button == -2)) {
            continue;
        }
        if (current_button_state.button == -1 ||
...
void ONScripter::buildDialog(bool yesno_flag,
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            dialog_font.top_xy[0] =
                rect.x +
                (rect.w - dialog_font.pitch_xy[0] * strlen(mes[i]) / 2) / 2;
            dialog_font.top_xy[1] =
                rect.y + (rect.h - dialog_font.font_size_xy[1]) / 2;
