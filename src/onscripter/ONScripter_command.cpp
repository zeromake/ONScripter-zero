...
int ONScripter::waveCommand() {
    ...
    if (script_h.isName("waveloop")) wave_play_loop_flag = true;
...
int ONScripter::talCommand() {
    ...
    if (no >= 0) trans = script_h.readInt();
...
int ONScripter::strspCommand() {
    ...
    if (ai->image_surface && ai->visible) dirty_rect.add(ai->pos);
    ...
    _FontInfo fi;
        ...
        calcUserRatio(calcFontSize(script_h.readInt(), fi.types));
    fi.font_size_xy[1] =
        calcUserRatio(calcFontSize(script_h.readInt(), fi.types));
    ...
    if (ai->num_of_cells == 0) {
        ...
        ai->color_list[0][0] = ai->color_list[0][1] = ai->color_list[0][2] =
            0xff;
    } else {
        ai->color_list = new uchar3[ai->num_of_cells];
...
int ONScripter::sp_rgb_gradationCommand() {
    ...
    for (i = upper_bound; i <= lower_bound; i++) {
        ...
        if (upper_bound != lower_bound) {
            color |= (((lower_r - upper_r) * (i - upper_bound) /
                           (lower_bound - upper_bound) +
                       upper_r) >>
                      fmt->Rloss)
                     << fmt->Rshift;
            color |= (((lower_g - upper_g) * (i - upper_bound) /
                           (lower_bound - upper_bound) +
                       upper_g) >>
                      fmt->Gloss)
                     << fmt->Gshift;
            color |= (((lower_b - upper_b) * (i - upper_bound) /
                           (lower_bound - upper_bound) +
                       upper_b) >>
                      fmt->Bloss)
                     << fmt->Bshift;
...
int ONScripter::spclclkCommand() {
    if (!force_button_shortcut_flag) spclclk_flag = true;
    return RET_CONTINUE;
}
...
int ONScripter::spbtnCommand() {
    ...
    if (script_h.isName("cellcheckspbtn")) cellcheck_flag = true;

    int sprite_no = script_h.readInt();
    int no = script_h.readInt();
    if (no < 1 || sprite_no < 0 || sprite_no >= MAX_SPRITE_NUM ||
...
void ONScripter::setwindowCore() {
    ...
    sentence_font.font_size_xy[0] =
        calcUserRatio(calcFontSize(script_h.readInt(), sentence_font.types));
    sentence_font.font_size_xy[1] =
        calcUserRatio(calcFontSize(script_h.readInt(), sentence_font.types));

    // 字符宽高
    sentence_font.pitch_xy[0] =
        script_h.readInt() + sentence_font.font_size_xy[0];
    sentence_font.pitch_xy[1] =
        script_h.readInt() + sentence_font.font_size_xy[1];
    ...
    if (buf[0] == '#') {
        ...
        sentence_font.window_color[0] = sentence_font.window_color[1] =
            sentence_font.window_color[2] = 0xff;
    }
    ...
    if (select_mode != SELECT_CSEL_MODE) {
        ...
        while (last_select_link) {
            if (*last_select_link->text) {
                ButtonLink *button = getSelectableSentence(
                    last_select_link->text, &sentence_font);
                root_button_link.insert(button);
                button->no = counter;
            }
            counter++;
...
int ONScripter::savetimeCommand() {
    ...
    if (!info.valid) {
        ...
        for (int i = 0; i < 3; i++) script_h.readVariable();
        return RET_CONTINUE;
    }
...
int ONScripter::savescreenshotCommand() {
    ...
    if (screenshot_surface == NULL)
        screenshot_surface = AnimationInfo::alloc32bitSurface(
            screen_device_width, screen_device_height, texture_format);

    SDL_Surface *surface = AnimationInfo::alloc32bitSurface(
        screenshot_w, screenshot_h, texture_format);
...
int ONScripter::savegameCommand() {
    ...
    if (script_h.isName("savegame2")) savegame2_flag = true;

    int no = script_h.readInt();

    const char *savestr = NULL;
    if (savegame2_flag) savestr = script_h.readStr();
...
int ONScripter::playCommand() {
    bool loop_flag = true;
    if (script_h.isName("playonce")) loop_flag = false;
    ...
    if (buf[0] == '*') {
        ...
        if (playSound(midi_file_name, SOUND_MIDI, midi_play_loop_flag) !=
            SOUND_MIDI) {
...
int ONScripter::mp3stopCommand() {
    if (Mix_PlayingMusic() == 1 && timer_bgmfade_id &&
        mp3fadeout_duration_internal > 0)  // already in fadeout
        return RET_CONTINUE;
    ...
    if (buf[0] != '\0') {
        if (buf[0] == '(') {
            ...
            while (*buf != ')' && *buf != '\0') {
                if (*buf >= '0' && *buf <= '9') {
                    if (integer_flag)
                        music_loopback_offset =
                            music_loopback_offset * 10.0 + *buf - '0';
                    else {
        ...
        if (mp3fadein_duration > 0) {
            ...
            mp3fade_start = SDL_GetTicks();
            timer_bgmfade_id =
                SDL_AddTimer(20, bgmfadeCallback, (void *)&timer_bgmfade_id);
            ...
            if (ext && (!strcmp(ext + 1, "OGG") || !strcmp(ext + 1, "ogg"))) {
                ...
                // wait until fadin is finished when playing music other than
                // ogg
                event_mode = WAIT_TIMER_MODE;
                waitEvent(-1);
            }
...
int ONScripter::lsp2Command() {
    ...
    if (ai->image_surface && ai->visible) dirty_rect.add(ai->bounding_rect);
    ai->visible = v;
    ...
    if (ai->visible) dirty_rect.add(ai->bounding_rect);
...
int ONScripter::logspCommand() {
    ...
    if (script_h.isName("logsp2")) logsp2_flag = true;
    ...
    ai->trans_mode = AnimationInfo::TRANS_STRING;
    if (logsp2_flag) {
        ai->font_size_xy[0] = calcUserRatio(
            calcFontSize(script_h.readInt(), ons_font::ANIM_FONT));
        ai->font_size_xy[1] = calcUserRatio(
            calcFontSize(script_h.readInt(), ons_font::ANIM_FONT));
        ai->font_pitch[0] =
            calcUserRatio(script_h.readInt()) + ai->font_size_xy[0];
        ai->font_pitch[1] =
            calcUserRatio(script_h.readInt()) + ai->font_size_xy[1];
    } else {
...
int ONScripter::loadgameCommand() {
    ...
    if (!loadSaveFile(no)) {
        ...
        if (loadgosub_label) gosubReal(loadgosub_label, script_h.getCurrent());
    }
...
int ONScripter::isdownCommand() {
    ...
    if (num_fingers > 1) current_button_state.down_flag = false;
#endif
...
int ONScripter::inputCommand() {
    ...
    utils::printInfo(
        "*** inputCommand(): $%d is set to the default value: %s\n", no, buf);
...
int ONScripter::humanorderCommand() {
    ...
    for (i = 0; i < 3; i++)
        if (tachi_info[i].image_surface) dirty_rect.add(tachi_info[i].pos);
...
int ONScripter::getscreenshotCommand() {
    ...
    if (screenshot_surface != nullptr &&
        (screenshot_surface->w != render_view_rect.w ||
         screenshot_surface->h != render_view_rect.h)) {
...
int ONScripter::getlogCommand() {
    ...
    if (script_h.isName("getlogtext")) getlogtext_flag = true;

    if (script_h.isName("getlog2")) getlog2_flag = true;
...
int ONScripter::getcursorposCommand() {
    ...
    if (fi.isEndOfLine()) {
        ...
        for (int i = 0; i < indent_offset; i++) fi.advanceCharInHankaku(2);
    }
...
int ONScripter::fileexistCommand() {
    ...
    script_h.setInt(&script_h.pushed_variable,
                    (script_h.cBR->getFileLength(buf) > 0) ? 1 : 0);
...
int ONScripter::exbtnCommand() {
    ...
    if (script_h.isName("exbtn_d") || script_h.isName("bdef")) {
        ...
        if (script_h.isName("cellcheckexbtn")) cellcheck_flag = true;
...
int ONScripter::drawsp3Command() {
    ...
    int denom =
        (ai->mat[0][0] * ai->mat[1][1] - ai->mat[0][1] * ai->mat[1][0]) / 1000;
    if (denom != 0) {
            ...
            if (btntime_value == -1) {
                if (automode_flag) {
                    ...
                    if (automode_time < 0) {
                        ...
                        if (t == -1 || t > automode_time) t = automode_time;
                    }
                    // current_button_state.button = 0;
...
int ONScripter::btndefCommand() {
    if (script_h.isName("bclear")) {
        ...
        if (buf[0] != '\0') {
            ...
            SDL_SetSurfaceBlendMode(btndef_info.image_surface,
                                    SDL_BLENDMODE_NONE);
        }
    }
...
static SDL_Texture *createMaximumTexture(
    ...
    blt_rect.w = blt_surface->w - src_rect.x > max_texture_width
                     ? max_texture_width
                     : blt_surface->w - src_rect.x;
    blt_rect.h = blt_surface->h - src_rect.y > max_texture_height
                     ? max_texture_height
                     : blt_surface->h - src_rect.y;
    blt_rect.x = src_rect.x;
    blt_rect.y = src_rect.y;
    SDL_Surface *surface = AnimationInfo::alloc32bitSurface(
        blt_rect.w, blt_rect.h, texture_format);
    ...
    if (sx >= 0 && sy >= 0 && sw > 0 && sh > 0) {
        ...
        if (blt_texture == NULL) {
            if (btndef_info.image_surface->w > max_texture_width ||
                ...
                blt_texture = SDL_CreateTextureFromSurface(
                    renderer, btndef_info.image_surface);
            }
        } else {
...
int ONScripter::bgCommand() {
    ...
    for (int i = 0; i < 3; i++) tachi_info[i].remove();
...
void ONScripter::NSDCallCommand(int texnum, const char *str1, int proc,
    ...
    if (proc == 1) {  // deffontd.dll, Font
        ...
        while (str2[i] && num_param < 8)
            if (str2[i++] == ',') start[num_param++] = str2 + i;
        switch (num_param) {
            case 8:
            case 7:
                for (i = 0; i < 2; i++) {
                    ...
                    while (start[4 + i][j] >= '0' && start[4 + i][j] <= '9')
                        ruby_struct.font_size_xy[i] =
                            ruby_struct.font_size_xy[i] * 10 +
                            start[4 + i][j++] - '0';
                }
            case 5:
                i = 0;
            ...
            case 3:
                for (i = 0; i < 2; i++) {
                    ...
                    while (start[i][j] >= '0' && start[i][j] <= '9')
                        f_info.font_size_xy[i] =
                            f_info.font_size_xy[i] * 10 + start[i][j++] - '0';
                }
        ...
        char *p = (char *)start[num_param - 1],
             *p2 = (char *)start[num_param - 1];
        while (*p) {
            if (IS_TWO_BYTE(*p)) {
                *p2++ = *p++;
                *p2++ = *p++;
...
void ONScripter::NSDLoadCommand(int texnum, const char *str) {
    ...
    if (str[0] != '*') {
        ...
        int c = 1, n = 0,
            val[6] = {0};  // val[6] = {width, height, R, G, B, alpha}
