...
bool ONScripter::doEffect(EffectLink *effect, bool clear_dirty_region) {
    ...
    switch (effect_no) {
        ...
        case 6:  // Left curtain
            width = EFFECT_STRIPE_CURTAIN_WIDTH * effect_counter * 2 /
                    effect_duration;
            for (i = 0; i <= screen_width / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
                width2 = width - EFFECT_STRIPE_CURTAIN_WIDTH *
                                     EFFECT_STRIPE_CURTAIN_WIDTH * i /
                                     screen_width;
                if (width2 >= 0) {
        ...
        case 7:  // Right curtain
            ...
            for (i = 0; i <= screen_width / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
                ...
                if (width2 >= 0) {
                    ...
                    src_rect.x =
                        screen_width - i * EFFECT_STRIPE_CURTAIN_WIDTH - width2;
                    src_rect.y = 0;
                    src_rect.w = width2;
                    src_rect.h = screen_height;
                    drawEffect(&src_rect, &src_rect, effect_dst_surface);
        ...
        case 8:  // Top curtain
            height = EFFECT_STRIPE_CURTAIN_WIDTH * effect_counter * 2 /
                     effect_duration;
            for (i = 0; i <= screen_height / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
                height2 = height - EFFECT_STRIPE_CURTAIN_WIDTH *
                                       EFFECT_STRIPE_CURTAIN_WIDTH * i /
                                       screen_height;
                if (height2 >= 0) {
                    src_rect.x = 0;
        ...
        case 9:  // Bottom curtain
            ...
            for (i = 0; i <= screen_height / EFFECT_STRIPE_CURTAIN_WIDTH; i++) {
                ...
                if (height2 >= 0) {
                    src_rect.x = 0;
                    src_rect.y = screen_height -
                                 i * EFFECT_STRIPE_CURTAIN_WIDTH - height2;
                    src_rect.w = screen_width;
                    src_rect.h = height2;
                    drawEffect(&src_rect, &src_rect, effect_dst_surface);
                }
        ...
        case 16:  // Mosaic out
            generateMosaic(effect_src_surface,
                           5 - 6 * effect_counter / effect_duration);
            break;

        case 17:  // Mosaic in
            generateMosaic(effect_dst_surface,
                           6 * effect_counter / effect_duration);
            break;
        ...
        case (MAX_EFFECT_NUM + 0):  // quakey
            ...
            amp =
                (Sint16)(sin(M_PI * 2.0 * effect->no * effect_counter /
                             effect_duration) *
                         EFFECT_QUAKE_AMP * effect->no *
        ...
        case (MAX_EFFECT_NUM + 3):  // flushout
            if (effect_counter > 0) {
                ...
                height = 30 * (effect_counter + effect_timer_resolution) /
                         effect_duration;
                if (height > width) {
                    int i, j, ii, jj;
    ...
    if (effect_counter < effect_duration && effect_no != 1) {
        ...
        if (effect_blank != 0 && effect_counter != 0) waitEvent(effect_blank);
...
void ONScripter::generateMosaic(SDL_Surface *src_surface, int level) {
    ...
    for (i = screen_height - 1; i >= 0; i -= width) {
        for (j = 0; j < screen_width; j += width) {
            ...
            ONSBuf *dst_buffer =
                (ONSBuf *)accumulation_surface->pixels + i * total_width + j;
