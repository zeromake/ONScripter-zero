...
void ONScripter::drawGlyph(SDL_Surface *dst_surface, _FontInfo *info,
    ...
    if (info->getTateyokoMode() == _FontInfo::TATE_MODE &&
...
void ONScripter::drawString(const char *str, uchar3 color, _FontInfo *info,
    ...
    if (flush_flag) flush(refresh_shadow_text_mode, &scaled_clipped_rect);
    ...
    for (int i = 0; i < current_page->text_count; i++) {
        if (current_page->text[i] == '\n') {
            ...
            if (out_text[0] == '(') {
                ...
                int no = 0;
                while (current_page->text[i + 1] >= '0' &&
                       current_page->text[i + 1] <= '9')
...
void ONScripter::enterTextDisplayMode(bool text_flag) {
    if (line_enter_status <= 1 && (!pretextgosub_label || saveon_flag) &&
        internal_saveon_flag && text_flag) {
        storeSaveFile();
...
bool ONScripter::clickWait(char *out_text) {
    ...
    if (script_h.checkClickstr(script_h.getStringBuffer() +
...
int ONScripter::textCommand() {
    ...
    if (pretextgosub_label &&
        ...
        (!pagetag_flag || page_enter_status == 0) && line_enter_status == 0) {
...
void ONScripter::processEOT() {
    ...
    if (!sentence_font.isLineEmpty() && !new_line_skip_flag) {
        // if sentence_font.isLineEmpty() is true, newPage() might be already
        // issued
...
bool ONScripter::processText() {
    ...
    if (IS_TWO_BYTE(ch)) {  // Shift jis
        /* ---------------------------------------- */
        /* Kinsoku process */
        if (checkLineBreak(script_h.getStringBuffer() + string_buffer_offset,
                           &sentence_font)) {
            sentence_font.newLine();
            current_page->add('\n');
            for (int i = 0; i < indent_offset; i++) {
        ...
        int wait_time = sentence_font.wait_time == -1
                            ? default_text_speed[text_speed_no]
        ...
        int matched_len = script_h.checkClickstr(
            script_h.getStringBuffer() + string_buffer_offset, true);
        if (matched_len > 0) {
            out_text[0] = script_h.getStringBuffer()[string_buffer_offset];
        ...
        if (script_h.getStringBuffer()[string_buffer_offset] == 's') {
            ...
            while (script_h.getStringBuffer()[string_buffer_offset] >= '0' &&
                   script_h.getStringBuffer()[string_buffer_offset] <= '9') {
                t = t * 10 + script_h.getStringBuffer()[string_buffer_offset] -
                    '0';
                string_buffer_offset++;
            }
            while (script_h.getStringBuffer()[string_buffer_offset] == ' ' ||
        ...
        return true;
               ...
               !(script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR)) {
        readColor(&sentence_font.color,
                  script_h.getStringBuffer() + string_buffer_offset);
        readColor(&ruby_font.color,
                  script_h.getStringBuffer() + string_buffer_offset);
        ...
        return true;
               ...
               (!english_mode ||
                !(script_h.getEndStatus() & ScriptHandler::END_1BYTE_CHAR))) {
        current_page->add('(');
        startRuby(script_h.getStringBuffer() + string_buffer_offset + 1,
                  sentence_font);
        ...
        string_buffer_offset =
            ruby_struct.ruby_end - script_h.getStringBuffer();
        if (*ruby_struct.ruby_end == ')') {
            if (skip_mode || ctrl_pressed_status)
        ...
        while (script_h.getStringBuffer()[string_buffer_offset] >= '0' &&
            ...
            no = no * 10 + script_h.getStringBuffer()[string_buffer_offset++] -
                 '0';
        }
        in_textbtn_flag = true;
        return true;
        ...
        int matched_len = script_h.checkClickstr(script_h.getStringBuffer() +
                                                 string_buffer_offset);
