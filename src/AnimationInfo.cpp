...
int AnimationInfo::doClipping(SDL_Rect *dst,
    ...
    if (!dst || dst->x >= clip->x + clip->w || dst->x + dst->w <= clip->x ||
...
#define BLEND_PIXEL()                                                     \
    do {                                                                  \
        ...
        Uint32 mask_rb =                                                  \
            (((((*src_buffer & 0xff00ff) - temp) * mask2) >> 8) + temp) & \
            0xff00ff;                                                     \
        temp = *dst_buffer & 0x00ff00;                                    \
        Uint32 mask_g =                                                   \
            (((((*src_buffer & 0x00ff00) - temp) * mask2) >> 8) + temp) & \
            0x00ff00;                                                     \
...
#define ADDBLEND_PIXEL()                                                       \
    do {                                                                       \
        ...
        mask_rb |=                                                             \
            ((mask_rb & AMASK) ? RMASK : 0) | ((mask_rb & GMASK) ? BMASK : 0); \
...
inline void rainAddBlendPixel32(const Uint32 *src_buffer,
                                Uint32 *__restrict dst_buffer) {
#ifdef USE_SIMD
    using namespace simd;
    ...
    struct Blender {
        ...
        void blendLine(Uint32 *line_buffer,
            ...
            if (blending_mode == BLEND_NORMAL) {
                ...
                if (size >= 4) {
                    ...
                    ivec256 zero = ivec256::zero();
                    uint8x32 mask =
                        uint8x32::set8(3, 7, 11, 15, 19, 23, 27, 31);
                    uint8x32 amask =
#if SDL_BYTEORDER == SDL_LIL_ENDIAN
                        uint8x32::set(0, 0, 0, 0xFF);
#else
                    ...
                    while (size >= 8) {
#if SDL_BYTEORDER == SDL_LIL_ENDIAN
                        unsigned char *alphap =
                            (unsigned char *)line_buffer + 3;
#else
        ...
        void operator()(const int y) const {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                int x = (corner_xy[i2][0] - corner_xy[i][0]) *
                            (y - corner_xy[i][1]) /
                            (corner_xy[i2][1] - corner_xy[i][1]) +
                        corner_xy[i][0];
            ...
            ONSBuf *dst_buffer_s =
                (ONSBuf *)dst_surface->pixels + dst_surface->w * y + raster_min;
            ...
            int line_pos = 0;
            for (int x = raster_min - dst_x; x <= raster_max - dst_x;
                 x++, dst_buffer++) {
...
SDL_Surface *AnimationInfo::setupImageAlpha(SDL_Surface *surface,
    ...
    if (trans_mode == TRANS_ALPHA && !has_alpha) {
    ...
    } else if (trans_mode == TRANS_TOPLEFT || trans_mode == TRANS_TOPRIGHT ||
        ...
        for (i = h; i != 0; i--) {
            for (j = w; j != 0; j--, buffer++, alphap += 4) *alphap = 0xff;
        }
    }
