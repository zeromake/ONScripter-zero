...
static MagickPassFail HorizontalFilter(
    ...
    unsigned char *source_pixels = (unsigned char *)source->pixels;
    unsigned char *destination_pixels = (unsigned char *)destination->pixels;
    ...
    for (x=0; x < (long) destination->w; x++) {
        ...
        for (n=0; n < (stop-start); n++)
        {
            ...
            contribution[n].weight = filter_info->function(scale*((double) start+n-center+0.5),filter_info->support);
        ...
        size_t p_offset = 4 * source->h * contribution[0].pixel;
        RGBAPixelPacket *p = (RGBAPixelPacket *)(source_pixels + p_offset);
        size_t q_offset = 4 * destination->h * x;
        RGBAPixelPacket *q = (RGBAPixelPacket *)(destination_pixels + q_offset);
            ...
            if (matte) {
                for (long y=0; y < (long) destination->h; y++) {
                    ...
                    for (i=0; i < n; i++)
                    {
                      ...
                      transparency_coeff = weight * (1 - ((double) p[jj].opacity/TransparentOpacity));
                    ...
                    long yy = (y % destination->h) * destination->h + (y / destination->h);
...
static MagickPassFail VerticalFilter(
    const SDL_Surface *restrict source,
    SDL_Surface *restrict destination,
    ...
    for (int y = 0; y < destination->h; y++) {
        ...
        size_t p_offset = 4 * source->w * contribution[0].pixel;
        RGBAPixelPacket *p = (RGBAPixelPacket *)(source_pixels + p_offset);
        size_t q_offset = 4 * destination->w * y;
        RGBAPixelPacket *q = (RGBAPixelPacket *)(destination_pixels + q_offset);
