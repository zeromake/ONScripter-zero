...
#define SFL_DTL_BEGIN \
    namespace dtl {   \
    namespace small_unordered_flat_map_dtl {
#define SFL_DTL_END \
    }               \
    }
...
SFL_DTL_BEGIN  /////////////////////////////////////////////////////////////////
    ...
    SFL_CONSTEXPR_14 void ignore_unused(Args&&...) {
    // Do nothing.
}
...
constexpr auto to_address(const Pointer& p) noexcept ->
...
void construct_at(Allocator& a, Pointer p, Args&&... args) {
    std::allocator_traits<Allocator>::construct(
        a, SFL_DTL::to_address(p), std::forward<Args>(args)...);
}

template <typename Allocator, typename Pointer>
void destroy_at(Allocator& a, Pointer p) noexcept {
    std::allocator_traits<Allocator>::destroy(a, SFL_DTL::to_address(p));
}
...
template <typename Allocator, typename ForwardIt, typename Size>
ForwardIt uninitialized_default_construct_n(Allocator& a,
                                            ForwardIt first,
                                            Size n) {
...
ForwardIt uninitialized_fill_n(Allocator& a,
                               ForwardIt first,
                               Size n,
                               const T& value) {
    ForwardIt curr = first;
...
ForwardIt uninitialized_copy(Allocator& a,
                             InputIt first,
                             InputIt last,
                             ForwardIt d_first) {
...
template <typename Allocator, typename InputIt, typename ForwardIt>
ForwardIt uninitialized_move(Allocator& a,
                             InputIt first,
                             InputIt last,
                             ForwardIt d_first) {
    ForwardIt d_curr = d_first;
    SFL_TRY {
        while (first != last) {
            SFL_DTL::construct_at(
                a, std::addressof(*d_curr), std::move(*first));
            ++d_curr;
            ++first;
        }
        return d_curr;
...
template <typename Allocator, typename InputIt, typename ForwardIt>
ForwardIt uninitialized_move_if_noexcept(Allocator& a,
                                         InputIt first,
                                         InputIt last,
                                         ForwardIt d_first) {
    ForwardIt d_curr = d_first;
    SFL_TRY {
        while (first != last) {
            SFL_DTL::construct_at(
                a, std::addressof(*d_curr), std::move_if_noexcept(*first));
            ++d_curr;
            ++first;
        }
        return d_curr;
...
struct has_is_transparent<Type,
                          SfinaeType,
                          void_t<typename Type::is_transparent>>
    : std::true_type {};
...
SFL_DTL_END  ///////////////////////////////////////////////////////////////////
   ...
   public:
    ...
    static_assert(
        ...
        "Allocator::value_type must be same as "
        "sfl::small_unordered_flat_map::value_type.");
   ...
   private:
    ...
    class data_base {
       private:
        alignas(
            value_type) unsigned char internal_storage_[N * sizeof(value_type)];
       ...
       public:
        ...
        data_base() noexcept
            : first_(std::pointer_traits<pointer>::pointer_to(
    ...
    class data_base<false, Dummy> {
       public:
        ...
        data_base() noexcept : first_(nullptr), last_(nullptr), end_(nullptr) {}

        pointer internal_storage() noexcept { return nullptr; }
    };

    class data : public data_base<(N > 0)>,
       ...
       public:
        data() noexcept(
            ...
            : allocator_type(), ultra_equal() {}

        data(const ultra_equal& equal) noexcept(
            std::is_nothrow_default_constructible<allocator_type>::value &&
            std::is_nothrow_copy_constructible<ultra_equal>::value)
            : allocator_type(), ultra_equal(equal) {}

        data(const allocator_type& alloc) noexcept(
            std::is_nothrow_copy_constructible<allocator_type>::value &&
            std::is_nothrow_default_constructible<ultra_equal>::value)
            : allocator_type(alloc), ultra_equal() {}

        data(const ultra_equal& equal, const allocator_type& alloc) noexcept(
            std::is_nothrow_copy_constructible<allocator_type>::value &&
            std::is_nothrow_copy_constructible<ultra_equal>::value)
            : allocator_type(alloc), ultra_equal(equal) {}

        data(ultra_equal&& equal, allocator_type&& alloc) noexcept(
            std::is_nothrow_move_constructible<allocator_type>::value &&
            std::is_nothrow_move_constructible<ultra_equal>::value)
            : allocator_type(std::move(alloc)), ultra_equal(std::move(equal)) {}

        data(ultra_equal&& equal, const allocator_type& alloc) noexcept(
            std::is_nothrow_copy_constructible<allocator_type>::value &&
            std::is_nothrow_move_constructible<ultra_equal>::value)
            : allocator_type(alloc), ultra_equal(std::move(equal)) {}

        allocator_type& ref_to_alloc() noexcept { return *this; }

        const allocator_type& ref_to_alloc() const noexcept { return *this; }

        ultra_equal& ref_to_equal() noexcept { return *this; }

        const ultra_equal& ref_to_equal() const noexcept { return *this; }
    };
   ...
   public:
    ...
    explicit small_unordered_flat_map(
        const KeyEqual& equal,
        const Allocator&
            alloc) noexcept(std::is_nothrow_copy_constructible<Allocator>::
                                value &&
                            std::is_nothrow_copy_constructible<KeyEqual>::value)
        : data_(equal, alloc) {}
    ...
    template <typename InputIt,
              typename std::enable_if<
                  SFL_DTL::is_input_iterator<InputIt>::value>::type* = nullptr>
    small_unordered_flat_map(InputIt first, InputIt last) : data_() {
    ...
    template <typename InputIt,
              typename std::enable_if<
                  SFL_DTL::is_input_iterator<InputIt>::value>::type* = nullptr>
    small_unordered_flat_map(InputIt first,
                             InputIt last,
    ...
    small_unordered_flat_map(const small_unordered_flat_map& other,
                             const Allocator& alloc)
        : data_(other.data_.ref_to_equal(), alloc) {
        initialize_copy(other);
    }
    ...
    small_unordered_flat_map(small_unordered_flat_map&& other,
                             const Allocator& alloc)
        : data_(std::move(other.data_.ref_to_equal()), alloc) {
        initialize_move(other);
    }

    ~small_unordered_flat_map() {
        SFL_DTL::destroy(data_.ref_to_alloc(), data_.first_, data_.last_);

        if (data_.first_ != data_.internal_storage()) {
            SFL_DTL::deallocate(
                data_.ref_to_alloc(), data_.first_, data_.end_ - data_.first_);
        }
    }
    ...
    small_unordered_flat_map& operator=(
    ...
    SFL_NODISCARD
    key_equal key_eq() const { return data_.ref_to_equal(); }
    ...
    SFL_NODISCARD
    value_equal value_eq() const { return value_equal(data_.ref_to_equal()); }
    ...
    SFL_NODISCARD
    iterator begin() noexcept { return data_.first_; }

    SFL_NODISCARD
    const_iterator begin() const noexcept { return data_.first_; }

    SFL_NODISCARD
    const_iterator cbegin() const noexcept { return data_.first_; }

    SFL_NODISCARD
    iterator end() noexcept { return data_.last_; }

    SFL_NODISCARD
    const_iterator end() const noexcept { return data_.last_; }

    SFL_NODISCARD
    const_iterator cend() const noexcept { return data_.last_; }
    ...
    SFL_NODISCARD
    bool empty() const noexcept { return data_.last_ == data_.first_; }

    SFL_NODISCARD
    size_type size() const noexcept { return data_.last_ - data_.first_; }
    ...
    SFL_NODISCARD
    size_type capacity() const noexcept { return data_.end_ - data_.first_; }
        ...
        if (new_cap > capacity()) {
            if (new_cap <= N) {
                if (data_.first_ == data_.internal_storage()) {
                    ...
                    SFL_DTL::destroy(
                        data_.ref_to_alloc(), data_.first_, data_.last_);

                    SFL_DTL::deallocate(data_.ref_to_alloc(),
                                        data_.first_,
                ...
                pointer new_first =
                    SFL_DTL::allocate(data_.ref_to_alloc(), new_cap);
                pointer new_last = new_first;
                ...
                SFL_CATCH(...) {
                    SFL_DTL::deallocate(
                        data_.ref_to_alloc(), new_first, new_cap);
    ...
    template <
        typename M,
        typename std::enable_if<
            std::is_assignable<mapped_type&, M&&>::value>::type* = nullptr>
    std::pair<iterator, bool> insert_or_assign(const Key& key, M&& obj) {
        auto it = find(key);

        if (it == end()) {
            return std::make_pair(
                insert_aux(std::piecewise_construct,
    ...
    template <
        typename K,
        typename std::enable_if<
            SFL_DTL::has_is_transparent<KeyEqual, K>::value>::type* = nullptr>
    size_type erase(K&& x) {
        auto it = find(x);
    ...
    void swap(small_unordered_flat_map& other) {
        ...
        SFL_ASSERT(allocator_traits::propagate_on_container_swap::value ||
        ...
        if (this->data_.first_ == this->data_.internal_storage() &&
            other.data_.first_ == other.data_.internal_storage()) {
            const size_type this_size = this->size();
            const size_type other_size = other.size();

            if (this_size <= other_size) {
                std::swap_ranges(this->data_.first_,
                                 this->data_.first_ + this_size,
                                 other.data_.first_);

                SFL_DTL::uninitialized_move(this->data_.ref_to_alloc(),
                                            other.data_.first_ + this_size,
                                            other.data_.first_ + other_size,
                                            this->data_.first_ + this_size);

                SFL_DTL::destroy(other.data_.ref_to_alloc(),
                                 other.data_.first_ + this_size,
                                 other.data_.first_ + other_size);
            } else {
                std::swap_ranges(other.data_.first_,
                                 other.data_.first_ + other_size,
                                 this->data_.first_);

                SFL_DTL::uninitialized_move(other.data_.ref_to_alloc(),
                                            this->data_.first_ + other_size,
                                            this->data_.first_ + this_size,
                                            other.data_.first_ + other_size);

                SFL_DTL::destroy(this->data_.ref_to_alloc(),
                                 this->data_.first_ + other_size,
                                 this->data_.first_ + this_size);
            }
        ...
        } else if (this->data_.first_ == this->data_.internal_storage() &&
                   other.data_.first_ != other.data_.internal_storage()) {
    ...
    template <
        ...
        typename std::enable_if<
            SFL_DTL::has_is_transparent<KeyEqual, K>::value>::type* = nullptr>
    SFL_NODISCARD iterator find(const K& x) {
        for (auto it = begin(); it != end(); ++it) {
    ...
    template <
        ...
        typename std::enable_if<
            SFL_DTL::has_is_transparent<KeyEqual, K>::value>::type* = nullptr>
    SFL_NODISCARD const_iterator find(const K& x) const {
        for (auto it = begin(); it != end(); ++it) {
            if (data_.ref_to_equal()(*it, x)) {
                return it;
            }
    ...
    template <
        ...
        typename std::enable_if<
            SFL_DTL::has_is_transparent<KeyEqual, K>::value>::type* = nullptr>
    SFL_NODISCARD size_type count(const K& x) const {
        return find(x) != end();
    }
    ...
    template <
        ...
        typename std::enable_if<
            SFL_DTL::has_is_transparent<KeyEqual, K>::value>::type* = nullptr>
    SFL_NODISCARD bool contains(const K& x) const {
        return find(x) != end();
    }
    ...
    SFL_NODISCARD
    value_type* data() noexcept { return SFL_DTL::to_address(data_.first_); }
   ...
   private:
    ...
    class temporary_value {
       private:
        ...
        value_type* buffer() { return reinterpret_cast<value_type*>(buffer_); }

       public:
        template <typename... Args>
        explicit temporary_value(allocator_type& a, Args&&... args)
            : alloc_(a) {
            SFL_DTL::construct_at(
                alloc_, buffer(), std::forward<Args>(args)...);
        }

        ~temporary_value() { SFL_DTL::destroy_at(alloc_, buffer()); }

        value_type& value() { return *buffer(); }
    };
    ...
    void initialize_copy(const small_unordered_flat_map& other) {
        ...
        SFL_TRY {
            data_.last_ = SFL_DTL::uninitialized_copy(data_.ref_to_alloc(),
                                                      other.data_.first_,
                                                      other.data_.last_,
                                                      data_.first_);
        }
    ...
    void initialize_move(small_unordered_flat_map& other) {
        if (other.data_.first_ == other.data_.internal_storage()) {
            data_.last_ = SFL_DTL::uninitialized_move(data_.ref_to_alloc(),
                                                      other.data_.first_,
                                                      other.data_.last_,
                                                      data_.first_);
    ...
    void assign_range(ForwardIt first, ForwardIt last) {
        ...
        if (n <= capacity()) {
            ...
            if (n <= s) {
                pointer new_last = std::copy(first, last, data_.first_);
                ...
                std::copy(first, mid, data_.first_);

                data_.last_ = SFL_DTL::uninitialized_copy(
                    data_.ref_to_alloc(), mid, last, data_.last_);
            }
        } else {
            reset(n);

            data_.last_ = SFL_DTL::uninitialized_copy(
                data_.ref_to_alloc(), first, last, data_.first_);
        }
    }
    ...
    void assign_move(small_unordered_flat_map& other) {
        ...
        if (other.data_.first_ == other.data_.internal_storage()) {
            assign_range(std::make_move_iterator(other.data_.first_),
    ...
    iterator insert_aux(Args&&... args) {
        ...
        if (data_.last_ != data_.end_) {
            SFL_DTL::construct_at(
                data_.ref_to_alloc(), data_.last_, std::forward<Args>(args)...);
            ...
            SFL_TRY {
                ...
                SFL_DTL::construct_at(data_.ref_to_alloc(),
                                      new_last,
                ...
                new_last = SFL_DTL::uninitialized_move_if_noexcept(
                    data_.ref_to_alloc(), data_.first_, data_.last_, new_last);
            }
            SFL_CATCH(...) {
                SFL_DTL::destroy(data_.ref_to_alloc(), new_first, new_last);
...
template <typename K, typename T, std::size_t N, typename E, typename A>
void swap(small_unordered_flat_map<K, T, N, E, A>& x,
...
template <typename K,
          ...
          typename A,
          typename Predicate>
typename small_unordered_flat_map<K, T, N, E, A>::size_type erase_if(
    small_unordered_flat_map<K, T, N, E, A>& c, Predicate pred) {
