 ...
 * \details LRU policy in the case of replacement removes the least recently
 * used element. That is, in the case of replacement necessity, that cache
 * policy returns a key that has not been touched recently. For example, cache
 * maximum size is 3 and 3 elements have been added - `A`, `B`, `C`. Then the
...
class LRUCachePolicy : public ICachePolicy<Key> {
   ...
   private:
    ...
    UnorderedMap<Key, lru_iterator> key_finder;
};
}  // namespace caches
