...
#include "small_unordered_flat_map.hpp"

namespace caches {
template <typename Key, typename Value>
using UnorderedMap = sfl::small_unordered_flat_map<Key, Value, 0>;

/**
 * \brief Fixed sized cache that can be used with different policy types (e.g.
 * LRU, FIFO, LFU) \tparam Key Type of a key (should be hashable) \tparam Value
 * Type of a value stored in the cache \tparam Policy Type of a policy to be
 * used with the cache \tparam HashMap Type of a hashmap to use for cache
 * operations. Should have `std::unordered_map` compatible interface
 */
template <typename Key,
          typename Value,
          template <typename> class Policy = NoCachePolicy,
          typename HashMap = UnorderedMap<Key, Value>>
class fixed_sized_cache {
   public:
     ...
     * \param[in] on_erase on_erase_cb function to be called when cache's
     * element get erased
     */
     ...
     * \return Pair of iterator that points to the element and boolean value
     * that shows whether get operation has been successful or not. If pair's
     * boolean value is false, the element is not presented in the cache. If
     * pair's boolean value is true, returned iterator can be used to get access
     * to the element
     */
     ...
     * \warning This method will change in the future with an optional class
     * capabilities to avoid throwing exceptions \throw std::range_error
     * \param[in] key Get element by key
   ...
   protected:
    void Clear() {
        ...
        std::for_each(
            begin(), end(), [&](const std::pair<const Key, Value> &el) {
