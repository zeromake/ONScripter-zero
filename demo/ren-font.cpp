...
int renwin_surface_scale(RenWindow* ren) { return ren->surface_scale; }
...
static int font_set_style(FT_Outline* outline,
    ...
    if (style & FONT_STYLE_BOLD) FT_Outline_EmboldenXY(outline, 1 << 5, 0);
...
RenFont* ren_font_load(const char* path,
    ...
    int len = 0;
    RenFont* font = NULL;
    if (FT_Set_Pixel_Sizes(face, 0, (int)(size * surface_scale))) goto failure;
    len = strlen(path);
    font = (RenFont*)check_alloc(calloc(1, sizeof(RenFont) + len + 1));
...
SDL_Surface* renwin_get_surface(RenWindow* ren) {
    SDL_Surface* surface = ren->windowSurface;
    if (!surface) {
        ...
        for (int i = 0; i < MAX_GLYPHSET; ++i) {
            ...
            if (!glyph_index ||
                ...
                font_set_style(&font->face->glyph->outline,
                               ...
                               font->style) ||
                FT_Render_Glyph(font->face->glyph,
                                (FT_Render_Mode)render_option)) {
                continue;
            }
            FT_GlyphSlot slot = font->face->glyph;
            int glyph_width = slot->bitmap.width / byte_width;
            if (font->antialiasing == FONT_ANTIALIASING_NONE) glyph_width *= 8;
            set->metrics[i] = GlyphMetric{
                (unsigned short)pen_x,
                (unsigned short)(pen_x + glyph_width),
                0,
                (unsigned short)slot->bitmap.rows,
                true,
                (short)slot->bitmap_left,
                (short)slot->bitmap_top,
                (slot->advance.x + slot->lsb_delta - slot->rsb_delta) / 64.0f};
            pen_x += glyph_width;
            font->max_height = slot->bitmap.rows > font->max_height
                                   ? slot->bitmap.rows
                                   : font->max_height;
            // In order to fix issues with monospacing; we need the unhinted
        ...
        for (int i = 0; i < MAX_GLYPHSET; ++i) {
            ...
            for (unsigned int line = 0; line < slot->bitmap.rows; ++line) {
                int target_offset = set->surface->pitch * line +
                                    set->metrics[i].x0 * byte_width;
                int source_offset = line * slot->bitmap.pitch;
                if (font->antialiasing == FONT_ANTIALIASING_NONE) {
                    for (unsigned int column = 0; column < slot->bitmap.width;
                         ++column) {
                        int current_source_offset =
                            source_offset + (column / 8);
                        int source_pixel =
                            slot->bitmap.buffer[current_source_offset];
                        pixels[++target_offset] =
                            ((source_pixel >> (7 - (column % 8))) & 0x1) << 7;
                    }
                } else
                    memcpy(&pixels[target_offset],
                           &slot->bitmap.buffer[source_offset],
                           slot->bitmap.width);
            }
        }
    }
...
static RenFont* font_group_get_glyph(GlyphSet** set,
    ...
    if (bitmap_index < 0) bitmap_index += SUBPIXEL_BITMAPS_CACHED;
    for (int i = 0; i < FONT_FALLBACK_MAX && fonts[i]; ++i) {
        *set = font_get_glyphset(fonts[i], codepoint, bitmap_index);
        *metric = &(*set)->metrics[codepoint % 256];
        if ((*metric)->loaded || codepoint < 0xFF) return fonts[i];
    }
    if (*metric && !(*metric)->loaded && codepoint > 0xFF &&
        codepoint != 0x25A1)
...
float ren_draw_text(RenFont** fonts,
    ...
    uint8_t* destination_pixels = (uint8_t*)surface->pixels;
    ...
    while (text < end) {
        ...
        if (set->surface && color.a > 0 && end_x >= clip.x &&
            ...
            for (int line = metric->y0; line < metric->y1; ++line) {
                int target_y = line + y - metric->bitmap_top +
                               font->baseline * surface_scale;
                ...
                for (int x = glyph_start; x < glyph_end; ++x) {
                    ...
                    // the standard way of doing this would be SDL_GetRGBA, but
                    // that introduces a performance regression. needs to be
                    // investigated
                    SDL_Color dst = {
                        (Uint8)((destination_color & surface->format->Rmask) >>
                                surface->format->Rshift),
                        (Uint8)((destination_color & surface->format->Gmask) >>
                                surface->format->Gshift),
                        (Uint8)((destination_color & surface->format->Bmask) >>
                                surface->format->Bshift),
                        (Uint8)((destination_color & surface->format->Amask) >>
                    ...
                    r = (color.r * src.r * color.a +
                         dst.r * (65025 - src.r * color.a) + 32767) /
                        65025;
                    g = (color.g * src.g * color.a +
                         dst.g * (65025 - src.g * color.a) + 32767) /
                        65025;
                    b = (color.b * src.b * color.a +
                         dst.b * (65025 - src.b * color.a) + 32767) /
                        65025;
                    // the standard way of doing this would be SDL_GetRGBA, but
                    // that introduces a performance regression. needs to be
                    // investigated
                    *destination_pixel++ = dst.a << surface->format->Ashift |
        ...
        if (!last)
            ...
            if (underline)
                ren_draw_rect(
                    RenRect{(int)last_pen_x,
                            (int)(y / surface_scale + last->height - 1),
                            (int)((local_pen_x - last_pen_x) / surface_scale),
                            (int)(last->underline_thickness * surface_scale)},
                    color);
            if (strikethrough)
                ren_draw_rect(
                    RenRect{(int)last_pen_x,
                            (int)(y / surface_scale + last->height / 2),
                            (int)((local_pen_x - last_pen_x) / surface_scale),
                            (int)(last->underline_thickness * surface_scale)},
                    color);
            last = font;
...
void ren_init(SDL_Window* win, SDL_Surface* windowSurface, int surface_scale) {
    ...
    draw_rect_surface = SDL_CreateRGBSurface(
