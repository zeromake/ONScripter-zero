...
void blendOver(png_bytep* dst, png_bytep* src, Uint32 x, Uint32 y, Uint32 w,
               Uint32 h) {
    Uint32 i, j;
    int u, v, al;
    for (j = 0; j < h; j++) {
...
#ifdef _MSC_VER
#pragma warning(                                                            \
    disable : 4611) /* warning C4611: interaction between '_setjmp' and C++ \
    ...
    if (color_type == PNG_COLOR_TYPE_GRAY) lib.png_set_expand(png_ptr);
    ...
    png_byte dop = 0;
    png_byte bop = 0;
    SDL_bool firstFrameIsHidden =
        png_get_first_frame_is_hidden(png_ptr, info_ptr) != 0;
    ...
    for (int frame_idx = 0; frame_idx < (anim->count); frame_idx++) {
        SDL_Surface* surface = SDL_CreateRGBSurface(
            SDL_SWSURFACE, width, height, bit_depth * num_channels, Rmask,
            Gmask, Bmask, Amask);
        if (surface == NULL) {
            error = SDL_GetError();
            goto done;
        ...
        if (ckey != -1) {
            if (color_type != PNG_COLOR_TYPE_PALETTE) {
                ...
                ckey = SDL_MapRGB(surface->format, (Uint8)transv->red,
        ...
        for (int row = 0; row < (int)height; row++) {
            row_pointers[row] =
                (png_bytep)(Uint8*)surface->pixels + row * surface->pitch;
        }
        png_read_frame_head(png_ptr, info_ptr);
        png_get_next_frame_fcTL(png_ptr, info_ptr, &w0, &h0, &x0, &y0,
                                &delay_num, &delay_den, &dop, &bop);
        if (frame_idx == first) {
            bop = PNG_BLEND_OP_SOURCE;
            for (int row = 0; row < (int)height; row++) {
                first_row_pointers[row] =
                    (png_bytep)(Uint8*)surface->pixels + row * surface->pitch;
            }
        }
...
int main(int argc, char* argv[]) {
    ...
    SDL_Texture* texture =
        SDL_CreateTextureFromSurface(renderer, firstImageSurface);
    ...
    while (!exit) {
        ...
        SDL_UpdateTexture(texture, &imgRect,
                          (unsigned char*)imageSurface->pixels,
                          imageSurface->pitch);
