...
RenFont* ren_font_load(const char* path, float size, ERenFontAntialiasing antialiasing, ERenFontHinting hinting, unsigned char style) {
  ...
  if (FT_Set_Pixel_Sizes(face, 0, (int)(size*surface_scale)))
    goto failure;
  if (FT_Set_Char_Size(face, 0, (int)(size*surface_scale) * 64, 72*surface_scale, 72*surface_scale))
    goto failure;
  len = strlen(path);
  ...
  for (int j = 0, pen_x = 0; j < bitmaps_cached; ++j) {
    ...
    for (int i = 0; i < MAX_GLYPHSET; ++i) {
      ...
      if (!glyph_index || FT_Load_Glyph(font->face, glyph_index, load_option | FT_LOAD_BITMAP_METRICS_ONLY)
        || font_set_style(&font->face->glyph->outline, j * (64 / SUBPIXEL_BITMAPS_CACHED), font->style) || FT_Render_Glyph(font->face->glyph, (FT_Render_Mode)render_option)) {
        continue;
      }
    ...
    if (pen_x == 0)
      continue;
    set->surface = (SDL_Surface*)check_alloc(SDL_CreateRGBSurface(0, pen_x, font->max_height, font->antialiasing == FONT_ANTIALIASING_SUBPIXEL ? 24 : 8, 0, 0, 0, 0));
    ...
    for (int i = 0; i < MAX_GLYPHSET; ++i) {
      ...
      if (FT_Render_Glyph(slot, (FT_Render_Mode)render_option))
        continue;
    ...
    if (set->surface && color.a > 0 && end_x >= clip.x && start_x < clip_end_x) {
      ...
      for (int line = metric->y0; line < metric->y1; ++line) {
        ...
        for (int x = glyph_start; x < glyph_end; ++x) {
          ...
          SDL_Color dst = { (Uint8)((destination_color & surface->format->Rmask) >> surface->format->Rshift), (Uint8)((destination_color & surface->format->Gmask) >> surface->format->Gshift), (Uint8)((destination_color & surface->format->Bmask) >> surface->format->Bshift), (Uint8)((destination_color & surface->format->Amask) >> surface->format->Ashift) };
          SDL_Color src;
...
int main() {
    ...
    Command cmd;
    cmd.fonts[0] = ren_font_load(DEFAUTL_TTF, 16, FONT_ANTIALIASING_SUBPIXEL, FONT_HINTING_SLIGHT, 0);
    while (!exit)
    {
        ...
        ren_draw_text(cmd.fonts, u8"test", 4, 10, 10, RenColor{0x00, 0x00, 0x00, 0xff});
